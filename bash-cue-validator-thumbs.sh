...
#~  Multiline parsing
    ...
    for p in $(grep -o "TRACK\|INDEX 01" "$1" | tr "\n" " " | sed 's/INDEX 01 /&\n/g')
        do [ "$p" = "TRACK INDEX 01 " ] || err "TRKI"
        done
    for p in $(grep -o "TRACK\|PREGAP\|INDEX 00" "$1" | tr "\n" " " | sed 's/TRACK/\n&/g')
        do [ "$p" = "TRACK PREGAP INDEX 00 " ] && err "GAP0"
        done
    for p in $(grep -o "TRACK\|INDEX .." "$1" | tr "\n" " " | sed 's/.\?TRACK /\n/g;s/INDEX //g;s/ $//')
        do [ "$(echo "$p" | tr " " "\n" | sort -uc 2>&1)" ] && err "IDXQ"
        done
    unset IFS
...
#~  Per line parsing
    ...
    do
        if [ "$WARN" != *" BO"* -a "$IDX" != "" ]; then
            [ "$(echo "$com" | grep -o " `echo "$line" | grep -o '^[A-Z]\+'` ")" ] || err "COMM"
        fi
    ...
    [ -f "$(grep -m1 FILE "$1" | cut -d\" -f2)" ] || err "IREF"
